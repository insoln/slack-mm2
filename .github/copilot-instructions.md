...
#### Performance / Import Tuning Environment Variables
These are optional and default to safe legacy behavior when unset:

- `IMPORT_BATCH_SIZE` — batch size for message import (messages stage). Default 200 (current code) if unset.
- `IMPORT_CHANNEL_CONCURRENCY` — parallel channel processing in messages stage. `1` = sequential (default). Increase carefully.
- `IMPORT_ORJSON_THRESHOLD_KB` — max file size (KB) to switch to orjson fast path for whole-file loads.
- `IMPORT_META_UPDATE_INTERVAL_SEC` / `IMPORT_META_UPDATE_EVERY` — throttling of meta/progress JSONB updates.
- `IMPORT_SINGLE_PASS` — if `1`, reactions/attachments/emojis collected during messages stage (skips later stages for them).
- `REACTIONS_BULK` — enable batched reaction import (`1`/`true`).
- `REACTIONS_BATCH_SIZE` — number of reactions per bulk INSERT when `REACTIONS_BULK=1` (e.g. 500 or 1000).
- `REACTIONS_PROGRESS_FLUSH_INTERVAL_SEC` — min seconds between progress callbacks in bulk mode (default 2.0).
