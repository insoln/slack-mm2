...
async def test_jobs_includes_stats_field():
    ...
    async with SessionLocal() as session:
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            assert (
                stats["types"] == expected_types
            ), f"Expected types {expected_types}, got {stats['types']}"
            ...
            assert (
                user_counts["success"] == 2
            ), f"Expected 2 success users, got {user_counts['success']}"
            assert (
                user_counts["failed"] == 1
            ), f"Expected 1 failed user, got {user_counts['failed']}"
            assert (
                user_counts["skipped"] == 0
            ), f"Expected 0 skipped users, got {user_counts['skipped']}"
            assert (
                user_counts["pending"] == 0
            ), f"Expected 0 pending users, got {user_counts['pending']}"

            # Verify message counts
            assert "message" in matrix, "message type missing from matrix"
            msg_counts = matrix["message"]
            assert (
                msg_counts["success"] == 3
            ), f"Expected 3 success messages, got {msg_counts['success']}"
            assert (
                msg_counts["failed"] == 0
            ), f"Expected 0 failed messages, got {msg_counts['failed']}"
            assert (
                msg_counts["skipped"] == 1
            ), f"Expected 1 skipped message, got {msg_counts['skipped']}"
            assert (
                msg_counts["pending"] == 1
            ), f"Expected 1 pending message, got {msg_counts['pending']}"

            # Verify attachment counts
            assert "attachment" in matrix, "attachment type missing from matrix"
            att_counts = matrix["attachment"]
            assert (
                att_counts["success"] == 1
            ), f"Expected 1 success attachment, got {att_counts['success']}"
            assert (
                att_counts["failed"] == 0
            ), f"Expected 0 failed attachments, got {att_counts['failed']}"
            ...
            assert all(
                emoji_counts[s] == 0 for s in stats["statuses"]
            ...
            assert (
                "export_status" in meta
            ), "export_status missing from meta (backward compatibility)"
            export_status = meta["export_status"]
            assert (
                export_status["user"]["success"] == 2
            ), "export_status should match stats"
...
async def test_jobs_stats_totals_calculation():
    ...
    async with SessionLocal() as session:
        ...
        entities = [
            Entity(
                ...
                status=(
                    ...
                    if i < 7
                    else MappingStatus.failed if i < 9 else MappingStatus.skipped
                ),
                data={},
            )
            for i in range(10)
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            success_total = sum(
                matrix[t]["success"] for t in ["message", "attachment"] if t in matrix
            )
            failed_total = sum(
                matrix[t]["failed"] for t in ["message", "attachment"] if t in matrix
            )
            skipped_total = sum(
                matrix[t]["skipped"] for t in ["message", "attachment"] if t in matrix
            )
            ...
            assert (
                success_total == 11
            ), f"Expected 11 total successes (7 msg + 4 att), got {success_total}"
            assert (
                failed_total == 3
            ), f"Expected 3 total failures (2 msg + 1 att), got {failed_total}"
            assert skipped_total == 1, f"Expected 1 skipped, got {skipped_total}"
            assert (
                completed_total == 15
            ), f"Expected 15 completed total, got {completed_total}"
