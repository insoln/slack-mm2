...
async def upload_backup(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    try:
        # Fast preflight: reject clearly non-zip by extension early
        name_lower = (file.filename or "").lower()
        if not name_lower.endswith(".zip"):
            return JSONResponse(
                status_code=400,
                content={"error": "Можно загружать только zip-архивы экспорта Slack"},
            )
        ...
        if size > MAX_UPLOAD_BYTES:
            backend_logger.error(
                f"UPLOAD: size {size} > MAX_UPLOAD_BYTES {MAX_UPLOAD_BYTES}, deleting {tmp_path}"
            )
            try:
                os.remove(tmp_path)
            except OSError:
        ...
        backend_logger.debug(
            f"Фоновый импорт Slack-экспорта: {tmp_path} ({size} bytes)"
        )
        ...
        return JSONResponse(
            status_code=500, content={"error": "Upload failed", "detail": str(e)}
        )
