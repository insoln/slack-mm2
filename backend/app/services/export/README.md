...
- ATTACHMENT_URL_TIMEOUT_SECONDS — расширенный таймаут (секунды) для скачивания/загрузки больших вложений через `/attachment_from_url`. По умолчанию 600. Используется только для этого эндпоинта; прочие запросы остаются на стандартном таймауте 30s.
...
### AttachmentExporter (дополнение)
Для больших файлов (видео, крупные PDF) стандартного 30s таймаута HTTP-клиента недостаточно — операция скачивания Slack + потоковая загрузка в Mattermost может занимать минуты. Чтобы избежать ложных `failed`:

* Если путь запроса оканчивается на `/attachment_from_url`, для него динамически формируется расширенный timeout: `connect=10s`, `read=WRITE=ext_seconds`, `pool=10s`, где `ext_seconds` берётся из `ATTACHMENT_URL_TIMEOUT_SECONDS` (по умолчанию 600).
* Ретраи: до 4 попыток для плагиновых POST (экспоненциальный backoff 0.5s, 1s, 2s) + дополнительный трёхкратный ретрай в самом AttachmentExporter.
* Рекомендация: устанавливать `ATTACHMENT_URL_TIMEOUT_SECONDS` пропорционально максимальному ожидаемому размеру файла (например 600–900 для видео ~150–200MB при средних сетевых условиях).
* Чтобы ограничить размер импорта без падений, можно задать `ATTACHMENT_MAX_MB` — тогда крупные файлы будут помечены `skipped`.
