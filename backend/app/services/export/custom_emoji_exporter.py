    ...
    async def export_entity(self):
        ...
        if emoji_name != original_name:
            backend_logger.debug(
                f"Транслитерировано название эмодзи: {original_name} -> {emoji_name}"
            )
        ...
        try:
            ...
            if resp.status_code != 200:
                backend_logger.error(
                    f"Не удалось скачать эмодзи: {emoji_url}, статус: {resp.status_code}"
                )
                await self.set_status(
                    "failed", error=f"Failed to download emoji: {resp.status_code}"
                )
                return
            ...
            if response.status_code in [200, 201]:
                ...
                backend_logger.debug(
                    f"Кастомный эмодзи {emoji_name} экспортирован в Mattermost"
                )
                return
            ...
            if err == "api.emoji.create.duplicate.app_error":
                ...
                if get_resp.status_code == 200:
                    ...
                    await self.set_status("success")
                    backend_logger.debug(
                        f"Эмодзи {emoji_name} уже существует в Mattermost, ID: {self.entity.mattermost_id}"
                    )
                    return

            backend_logger.error(
                f"Ошибка загрузки эмодзи в Mattermost: {response.status_code}, {response.text}"
            )
