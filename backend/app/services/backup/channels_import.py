...
async def parse_channels_and_chats(extract_dir):
    files = [
        "channels.json",
        "dms.json",
        "mpims.json",
    ...
    for fname in files:
        ...
        if os.path.exists(path):
            ...
            for channel_json in data:
                ...
                mattermost_id = None
                channel = Channel(slack_id=slack_id, mattermost_id=mattermost_id, raw_data=channel_json, auto_save=False)
...
def find_channel_for_folder(export_dir, _):
    folders = [f for f in os.listdir(export_dir) if os.path.isdir(os.path.join(export_dir, f))]
    all_channels = []
