...
* Конкурентность по каналам зафиксирована на значении 1 (последовательная обработка) для предотвращения гонок состояний в БД.
...
2. Публичные каналы: активный `public-channel` (`C0001`) и архивный `archived-public-reactions` (`CARCH1`), где `is_archived=true` и есть пост с реакциями.
3. Приватные каналы: активный `private-channel` (`G0001`) и архивный `archived-private-reactions` (`GARCH1`) для тестирования временного unarchive / rearchive цикла перед экспортом реакций.
4. Личный диалог (DM `D0001`).
5. По два дня активности (2025-01-01, 2025-01-02) в каждом базовом канале/DM; архивные reaction-каналы содержат отдельный день 2025-01-03 только для сценариев с реакциями.
...
8. Реакции (`thumbsup`, `eyes`, `white_check_mark`) без кастомных эмодзи на архивах публичного и приватного каналов.
