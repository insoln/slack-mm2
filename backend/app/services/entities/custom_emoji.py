...
async def get_slack_emoji_list():
    ...
    if not slack_token:
        backend_logger.warning("SLACK_BOT_TOKEN не настроен, пропускаю получение списка эмодзи")
        return {}
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            backend_logger.info(f"Получен список эмодзи из Slack API: {len(emoji_list)} эмодзи")
            return emoji_list
