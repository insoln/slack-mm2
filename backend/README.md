...
#### Валидация размера вложений
Система выполняет preflight-проверки размера файлов перед попыткой загрузки для избежания траты времени/трафика:

1. **Проверка лимита Mattermost** (приоритет): Backend запрашивает `FileSettings.MaxFileSize` из Mattermost через эндпоинт плагина `/config/max_file_size` (по умолчанию 100 МБ). Файлы, превышающие этот лимит, помечаются `skipped` до отправки в плагин.

2. **Проверка на стороне плагина**: Если `Content-Length` в ответе от Slack превышает `MaxFileSize`, плагин немедленно возвращает HTTP 413 без скачивания. При streaming используется кастомный `limitedReader`, который делает жёсткий лимит на объём данных и возвращает ошибку при превышении, чтобы избежать молчаливого усечения файлов.

3. **Legacy переменная** `ATTACHMENT_MAX_MB` (опционально): Устаревший лимит для обратной совместимости. Если задан, файлы больше указанного размера (МБ) пропускаются. Лимит Mattermost имеет приоритет.

**Категории ошибок**: При сбое экспорта вложения, сообщение об ошибке включает префикс категории для облегчения диагностики:
- `[file_too_large]` — файл превышает `MaxFileSize`
- `[timeout]` — истёк таймаут при загрузке/выгрузке файла
- `[mm_server_error]` — ошибка 5xx от Mattermost
- `[connection_error]` — проблемы соединения
- `[plugin_error]` — прочие ошибки плагина (4xx, validation и т.д.)
