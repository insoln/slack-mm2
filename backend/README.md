...
2. **Проверка на стороне плагина**: Если `Content-Length` в ответе от Slack превышает `MaxFileSize`, плагин немедленно возвращает HTTP 413 без скачивания. При streaming используется `io.LimitReader` для hard-cap на максимальный объём данных.
