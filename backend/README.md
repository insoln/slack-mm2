...
Импорт теперь использует фиксированные константы вместо переменных окружения для управления логикой:

- **Параллелизм каналов**: Зафиксирован на значении 1 (последовательная обработка) для предотвращения гонок состояний в БД.
- **Запись длительности стадий**: Управляется уровнем логирования DEBUG модуля `backend_logger`.
- **Интервал обновления метаданных**: Зафиксирован на 2 секунды между обновлениями прогресса.
- **Частота обновления метаданных**: Обновление происходит каждый batch_size сообщений.

Переменные окружения (`IMPORT_CHANNEL_CONCURRENCY`, `IMPORT_RECORD_STAGE_DURATIONS`, `IMPORT_META_UPDATE_INTERVAL_SEC`, `IMPORT_META_UPDATE_EVERY`) больше не поддерживаются и игнорируются.
...
* Для расследования узких мест установите уровень логирования DEBUG для модуля backend и смотрите `job.meta.durations`.
