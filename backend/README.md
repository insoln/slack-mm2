...
- Инвариант: каждое сообщение, относящееся к поддерживаемому канальному контексту, всегда имеет связь `posted_in` (гарантируется текущим импортёром).
...
- Частота записи всех счётчиков регулируется единым `IMPORT_PROGRESS_FLUSH_INTERVAL_SEC` (пер-типовых переменных больше нет).
