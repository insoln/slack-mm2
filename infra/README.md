...
**Для разработчиков**: См. [Developer Documentation](../docs/dev.md) для полной информации по настройке локальной среды разработки, Docker Compose и тестированию.
...
### Плагин: Эфемерная сборка и доставка
Сервис `plugin-autobuild` (одноразовый) условно собирает bundle `mm-importer-<version>.tar.gz` только если он отсутствует в `infra/test-data/plugin-bundles/`.

* Артефакт монтируется в backend как `/plugin-bundles:ro` (исключает перезапись исходников).
* Backend умеет скачивать bundle по `PLUGIN_BUNDLE_URL` в /tmp при отсутствии локального (опционально).
* Бинарник Go собирается статически (`CGO_ENABLED=0`) для совместимости с минимальным образом Mattermost.
* Создаётся sha256 файл и атомарный симлинк `latest.tar.gz`.

Причины подхода: минимальные права, повторяемость, отсутствие «грязи» в дереве исходников и возможность CDN-поставки.

### Удаление легаси (плагин)
Исключены: `build-dev.sh`, `Makefile`, авто-build на старте backend. Документация обновлена, остаётся только многостадийный Docker build и условный builder сервис. См. `infra/plugin/README.md` для деталей.
